...
void CGameTimer::Tick(float fLockFPS)
{
	...
	if (fLockFPS > 0.0f) {
		...
		// 파라미터 fLockFPS 가 0보다 크면 일정 시간 간격까지 함수를 기다리게 한다.
	...
	*/
	...
	// 초당 프레임 수를 1 증가시키고 현재 프레임 처리 시간을 누적해 둔다.
	m_nFramesPerSecond++;
	...
	// 누적된 프레임 처리 시간들의 평균을 구하여 평균 프레임 시간으로 삼는다.
	m_fTimeElapsed = 0.0f;
...
{
	// 현재 프레임 레이트를 문자열로 변환해 lpszString 버퍼에 채우고 " FPS)" 를 덧붙인다.
	if (lpszString)	{
