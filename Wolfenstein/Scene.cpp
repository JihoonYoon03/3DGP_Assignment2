...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	// 각 씬에 해당하는 셰이더를 미리 생성해두고 인스턴스를 보관한다.
...
{
	// 맵 선택 미니어처가 매번 회전 (현재 상태와 무관하게, MAP_SELECT 에서만 의미 있음).
	m_fMiniatureAngle += fTimeElapsed * 0.6f; // rad/sec

	// 현재 활성화된 씬의 객체만 애니메이션한다.
...
namespace {
	// MAP_SELECT 화면에서 두 개의 맵 미니어처가 좌우에 회전하며 떠 있도록 배치하는 데 사용하는 상수들.
	constexpr float kMiniOffsetX = 28.0f;       // 좌우 중심으로부터의 X 오프셋
	constexpr float kMiniScale   = 0.25f;       // 30x30 맵(120 단위)을 ~21 단위로 축소
	constexpr float kHoverScale  = 1.18f;       // 호버 시 추가 확대
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_eCurrentState == SceneState::MAP_SELECT) {
		// MAP_SELECT 에서는 활성 씬이 따로 없고 미니어처만 그려, MAP1/MAP2 의 미니어처를 보여준다.
	...
	// 게임플레이 씬에서 TPS 모드일 때 플레이어 모델을 같이 그린다.
	// 위에서 m_vShaders[MAP*].Render 가 셰이더와 루트 시그니처 PSO 를 바인딩해 두었으므로,
	// CGameObject::Render 의 자체 셰이더 없이 모델만 추가로 그릴 수 있다.
