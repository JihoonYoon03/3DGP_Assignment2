...
void CTextStringObject::UpdateBoundingBox() {
	...
	// CTextLetterMesh 는 기준점 (px, py) 에서 +X 방향으로 LETTER_GRID_COLS * pixelSize 만큼,
	// -Y 방향으로 LETTER_GRID_ROWS * pixelSize 만큼 글리프를 펼친다.
	// 따라서 글자 폭과 높이를 더해 정확한 바운딩 박스 크기로 보정해 둔다.
...
XMFLOAT3 CButtonObject::UnprojectScreenToWorld(int nMouseX, int nMouseY, int nScreenWidth, int nScreenHeight,
	...
	// inverse(view * proj) 로 클립 공간 좌표를 다시 월드 공간으로 되돌린다.
	XMMATRIX xmInvViewProj = XMMatrixInverse(nullptr, XMMatrixMultiply(xmView, xmProj));

	// 근평면(z=0) 과 원평면(z=1) 위의 점을 각각 월드 좌표로 변환해 두 점을 잇는 직선을 만든다.
	...
	// 그 직선과 z=0 평면의 교점이 결과가 된다. 화면 텍스트가 모두 z=0 평면 위에 있으므로 충분하다.
